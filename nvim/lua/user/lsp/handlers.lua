...
local function lsp_keymaps(bufnr)
  ...
  vim.api.nvim_buf_set_keymap(bufnr, 'n', 'gf', '<cmd>Lspsaga show_line_diagnostics<CR>', opts)
  vim.api.nvim_buf_set_keymap(bufnr, 'n', 'gsh', '<cmd>Lspsaga signature_help<CR>', opts)
...
M.on_attach = function(client, bufnr)
  -- tsserver 使用 prettier 进行格式化 不用 tsserver 自带的格式化工具
  if client.name == 'tsserver' then
    client.server_capabilities.documentFormattingProvider = false
  end

  -- sumneko_lua 使用 stylua 进行格式化 不用 sumneko_lua 自带的格式化工具
